...
### Fixed
- ABC 악보 스튜디오가 **abc 칸이 입력 소켓으로 바뀐 노드**에서도 제대로 들어갑니다.
  ComfyUI 에서 텍스트 위젯을 입력으로 바꾸고 선을 꽂으면 노드 자신의 위젯 값은
  무시되는데, 이전 버전은 그 위젯에만 값을 써서 악보가 조용히 묻혔습니다. 이제
  선을 따라가 근원 노드의 텍스트 칸에 넣습니다. YuE2_Music 워크플로처럼 `abc`
  칸에 문자열 노드를 꽂아 쓰는 구성이 여기 해당합니다.
- 넣을 칸을 못 찾으면 창을 닫지 않고 이유를 띄웁니다. 닫아 버리면 그린 악보가
  사라집니다.
- 선이 꽂힌 노드가 꺼져 있으면(뮤트·바이패스) 값을 넣은 뒤 그 사실을 알립니다.
  꺼진 노드의 출력은 생성 노드에 닿지 않습니다.
- abc 위젯이 입력 소켓으로 완전히 바뀌어 위젯 목록에서 사라진 노드에도 버튼이
  계속 붙습니다.

### Added
- 프런트엔드 단위 테스트를 CI 에 넣었습니다 (`tests/js/*.test.mjs`).

## [0.5.0] - 2026-09-11

### Added
- **ABC 악보 스튜디오.** `abc` 텍스트 위젯을 가진 노드 위에 **🎼 악보 스튜디오 열기**
  버튼이 붙습니다. 누르면 ComfyUI 안에 피아노롤 편집기가 뜨고, 그린 멜로디가
  ABC 악보 텍스트로 그 위젯에 그대로 들어갑니다. 노드 이름을 박아두지 않아서
  YuE2 · Generate Song 처럼 `abc` 칸을 가진 노드면 무엇이든 동작합니다.
- 편집기는 설치가 필요 없는 단독 HTML 한 장입니다. 도레미가 적힌 건반, 그리기·선택·
  지우개 도구, 곡 전체 미니맵, 코드 반주 재생, 가로세로 동시 확대를 갖췄습니다.
- **허밍으로 그리기.** 마이크로 흥얼거리거나 음원 파일 구간을 넣으면 YIN 피치
  검출로 멜로디를 따서 그려 넣습니다. 조와 박자에 맞춰 다듬고, 음색이 뚜렷하게
  갈리면 흥얼거린 구간과 가사로 부른 구간을 서로 다른 성부로 나눠 넣습니다.
  단선율 전용입니다. 드럼과 화음이 섞인 완성곡은 정확하지 않습니다.
- 보조 노드 `ABC 악보 입력`(악보 보관·재사용)과 `ABC 악보 정보`(조·박자·템포·마디 수).

### Note
- YuE2에 악보를 넘길 때는 그 노드의 `planning`을 `full` 또는 `melody`로 두어야
  합니다. `off`이면 YuE2 쪽에서 오류를 냅니다.
